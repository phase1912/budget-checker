...
Стоимость относительно заявленного аппетита (месяц+): укладывается — два деплой-пайплайна и настройка CORS занимают дни, не недели, оставшееся время уходит на самую архитектуру/схему БД.
...
- Полноценная авторизация не понадобится на этом этапе (черновая схема БД включает users, но без реального auth-флоуа) — *только выяснится позже*.
